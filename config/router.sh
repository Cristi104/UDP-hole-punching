...
# waiting for interfaces to show up
sleep 1
...
# mask local ips as the public router ip
iptables -t nat -A POSTROUTING -o $UPSTREAM_IFACE -s 192.168.0.0/16 -j MASQUERADE

# let through outbound packets but only allow inbound if a connection was created previously
